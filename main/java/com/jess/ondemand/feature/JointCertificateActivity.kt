...
class JointCertificateActivity : BaseSplitActivity() {

    private lateinit var installConfirmDialogLauncher: ActivityResultLauncher<IntentSenderRequest>

    private val onDemandJointCertificate: OnDemandDelivery by lazy {
        OnDemandDeliveryImpl(
            OnDemandModuleName.JOINT_CERTIFICATE,
        )
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        lifecycle.addObserver(onDemandJointCertificate)
    ...
    private fun initView() {
        ...
        findViewById<TextView>(R.id.title).text = OnDemandModuleName.JOINT_CERTIFICATE.name()

        findViewById<Button>(R.id.bt_joint_certificate).setOnClickListener {
            onDemandJointCertificate.loadAndLaunchModule()
        }
    }
    ...
    private fun initOnDemand() {
        onDemandJointCertificate.setOnSplitInstallListener(onInstallEvent = { event ->
            when (event.status) {
                SplitInstallSessionStatus.REQUIRES_USER_CONFIRMATION -> {
                    event.state?.let { state ->
                        onDemandJointCertificate.splitInstallManager.startConfirmationDialogForResult(
                            state,
